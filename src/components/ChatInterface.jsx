        ...
        <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
          ...
          <div {...getRootProps()} className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 ${isTextareaExpanded ? 'chat-input-expanded' : ''}`}>
            ...
            <textarea
              ...
              disabled={isLoading}
              rows={1}
              className="chat-input-placeholder w-full pl-12 pr-12 py-2 bg-transparent rounded-2xl focus:outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 disabled:opacity-50 resize-vertical min-h-[48px] max-h-[50vh] overflow-y-auto text-sm sm:text-base transition-all duration-200"
              style={{ 
                height: localStorage.getItem('chatInputHeight') || '48px',
                minHeight: '48px',
                maxHeight: '50vh'
              }}
              onMouseUp={(e) => {
                // Save height preference when user finishes resizing
                localStorage.setItem('chatInputHeight', e.target.style.height);
              }}
            />
            ...
            {input.trim() && (
              <button
                ...
                className="absolute right-14 top-1/2 -translate-y-1/2 w-5 h-5 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center transition-all duration-200 group z-10 shadow-sm"
                title="Clear input"
              >
                <svg 
                  className="w-3 h-3 text-gray-600 dark:text-gray-300 group-hover:text-gray-800 dark:group-hover:text-gray-100 transition-colors" 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
            ...
            {/* Icon container - stacked vertically on left */}
            ...
            {showInfoTooltip && (
              <div className="absolute left-10 bottom-full mb-2 p-3 bg-gray-900 dark:bg-gray-800 text-white text-xs rounded-lg shadow-lg z-50 whitespace-nowrap">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 transform -translate-y-1/2 w-10 h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed rounded-full flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:ring-offset-gray-800"
            >
              <svg 
